function Database(numberOfItems) {
    ...
    function randomSummary() {
        return randomElement([ 'Design ', 'Create ', 'Implement ', 'Remove ', 'Try to come up with some ' ]) +
            randomElement([ 'new ', 'better ', 'improved ', 'obsolete ' ]) +
            randomElement([ 'system ', 'machine ', 'process ', 'server ', 'application ' ]) +
            'for ' +
    ...
    for (var i = 0; i < numberOfItems; i++) {
        data.push({
            ...
            'team': randomElements(Database.randomUsers, randomInt(1, 4))
        });
    }
    this.data = data;
}

Database.prototype.sort = function(sortProperty, sortDirection) {
...
Database.prototype.getData = function(index, count)  {
